...
Patched for safer file uploads, boolean parsing, and clearer behavior.
"""
...
SIGNATURE_DIR = BASE_UPLOAD_DIR / "signatures"
QR_DIR = BASE_UPLOAD_DIR / "qr_codes"

# ensure directories exist
for directory in [AVATAR_DIR, LOGO_DIR, SIGNATURE_DIR, QR_DIR]:
    directory.mkdir(parents=True, exist_ok=True)

# allowed extensions & size
ALLOWED_IMAGE_EXT = {".png", ".jpg", ".jpeg", ".gif", ".webp", ".svg"}
MAX_UPLOAD_BYTES = 4 * 1024 * 1024  # 4 MB, adjust if needed
...
def save_upload_file(upload_file: UploadFile, directory: Path, prefix: str) -> str:
    """
    Save uploaded file securely and return web path like:
      /static/uploads/<folder>/<filename>
    Raises HTTPException on problems (size, extension, type).
    """
    ...
    # write in chunks, enforce max size
    bytes_written = 0
    try:
    ...
    # return web path, consistent with StaticFiles at /static -> app/static
    rel = Path("uploads") / directory.name / safe_name
...
async def upload_avatar(
    ...
    """Upload user avatar securely and return JSON with web path."""
    try:
        # Save file (will validate extension and size)
        avatar_web_path = save_upload_file(avatar, AVATAR_DIR, f"user_{user.id}")
...
async def upload_shop_logo(
    ...
    """Upload shop logo securely."""
    try:
        shop = getattr(user, "shop", None)
        if not shop:
            raise HTTPException(status_code=404, detail="Shop not found")

        logo_web_path = save_upload_file(logo, LOGO_DIR, f"shop_{shop.id}")
...
async def upload_signature(
    ...
    """Upload digital signature image for invoices."""
    try:
        shop = getattr(user, "shop", None)
        if not shop:
            raise HTTPException(status_code=404, detail="Shop not found")

        sig_web_path = save_upload_file(signature, SIGNATURE_DIR, f"shop_{shop.id}")
...
async def update_bank_details(
    ...
    try:
        ...
        # If so, do NOT update the encrypted account number (keep existing)
...
async def upload_qr_code(
    ...
    """Upload UPI QR code and attach to bank details."""
    try:
        shop = getattr(user, "shop", None)
        if not shop:
            raise HTTPException(status_code=404, detail="Shop not found")

        qr_web_path = save_upload_file(qr_code, QR_DIR, f"shop_{shop.id}")
